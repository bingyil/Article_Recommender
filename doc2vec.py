...
def load_articles(articles_dirname, gloves):
    ...
    for file in flist:
        ...
        articles.append((file[len(articles_dirname)+1:], title, contents, centroid))
    return articles
...
def recommended(article, articles, n):
    ...
    """
    dis_a = distances(article, articles)
    sorted_rec = sorted(dis_a, key=lambda x: x[0])[:n]
    ret = []
    for s in sorted_rec:
        ret.append(s[1])
    return ret
